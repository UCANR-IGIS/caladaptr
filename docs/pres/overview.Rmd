...
## Custom Visualization 
 ...
 ![](images/voxel_694x540.jpg){style="display:block; margin:1em auto; width:560px; border:1px solid grey;"}
...
![](images/hourly_temps_plot_600x449.png){style="display:block; margin:1em auto; border:1px solid grey;"}
 
## Custom decision support
 
![](images/mike-crimmins_spi-tool_1386x926.png){style="display:block; margin:1em auto; width:1000px; border:1px solid grey;"}
...
2) Convert units from Kelvin to &#176;F
...
In this example, we'll interpolate hourly temperature values from daily min-max. 


## 1\. Import the boundary of the study area

Our study area is [Lindcove Research and Extension Center](http://lrec.ucanr.edu/){target="_blank" rel="noopener"}, one of nine [field stations](http://recs.ucanr.edu/){target="_blank" rel="noopener"} maintained by UC ANR.

```{r}
lrec_sf <- st_read("lrec_bnd.geojson") %>% slice(1)
```
...
lrec_tempf_tbl <- cap_lrec_tbl %>%
  mutate(temp_f = set_units(val, degF))
head(lrec_tempf_tbl)
```

Next, we'll add additional columns required by the `make_hourly_temps()` in `chillR`.
...
lrec_tempf_dyr_tbl <- lrec_tempf_tbl %>%
  ...
  mutate(Year = as.integer(year(DATE)), Month = as.integer(month(DATE)), Day = day(DATE)) %>%
  mutate(temp_f = as.numeric(temp_f)) %>%
  select(cvar, temp_f, DATE, Year, Month, Day) %>%
...
## 5\. Analysis - Compute Hourly Temp

The `chillR` package has a function `make_hourly_temps()` that models hourly temperature based on sunrise and sunset times (which in turn are determined by date and latitude). Following the excellent  [guide](https://cran.r-project.org/web/packages/chillR/vignettes/hourly_temperatures.html) by author Eike Luedeling, we can model hourly temp in just a few step.
...
The first thing we need is the latitude of the study area (which chillR uses to compute sunrise and sunset):
...
To plot these, we need to convert the wide format to long:
...
![](images/q-and-a_400x256.png){style="display:block; margin:1em auto; margin-top:3em;"}

<p style='text-align:center; margin-top:5em;'><img src='images/igis-anr_horiz-stacked_320x80.png' style='height:80px;'/><img src='images/gif-logo-col_300x118.png' style='height:75px; margin-left:100px;'/><img src='images/cec_logo_400x354x256.png' style='height:100px; margin-left:100px;'/><img src='images/scg-logo_460x230.png' style='height:100px; margin-left:100px;'/></p>
