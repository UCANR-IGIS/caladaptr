...
2a: Select one or more downscaled climate projection from Scripps using **all 4** of these:
...
*Or* pick *any* of the 830 raster series datasets by their '**slug**':
...
number of consecutive days over a threshhold
Retrive Average Max Temperature for Point for 4 Priority GCMs and RCP45, Plot over time

(mimic the climate snapshot tool)


# Example 2: Grab from a Slug
...
# Example 4: Interpolate hourly temperature
...
Querying a lot of data? Use `ca_getvals_db()`.

Requires passing a filename for a local SQLIte database.

Progress bar shows how many calls remain

Connection interrupted? Picks up where it left off

Result is a R table linked to the SQLite database. Use same dplyr commands with some differences.
...
sample R notebooks on 
this and future presentations

Submit an Idea for a Code Recipe 

GitHub Issues

![](images/caladaptr-logo_beta.svg){style="width:400px; margin:1em auto;"}
