...
Let's plot the difference between rcp85 and rcp45 over time for one of the counties. First we filter the rows to one county, compute the temp in Fahrenheit, and put the rcp85 and rcp45 values into separate columns:
...
```{r}
census_all_sf <- ca_aoipreset_geom("censustracts")
census_all_sf
## Plotting all of the census tracts can take a few seconds:
## plot(census_all_sf %>% st_geometry())
```

Find those census tracts in Imperial County:

```{r}
tract_in_imperial_yn <- st_within(census_all_sf, counties_imperial_sf, sparse = FALSE)
table(tract_in_imperial_yn)

tm_shape(census_all_sf %>% filter(tract_in_imperial_yn)) +
  tm_polygons() +
...
```{r}
tracts_vals <- census_all_sf$tract[tract_in_imperial_yn]
tracts_vals
```
...
```{r}
cap3_tbl <- cap3 %>% ca_getvals() %>% ca_vals2tbl()
cap3_tbl
```
Pull out just the values for March and April:
...
Count the number of heat spells consisting of 3 or more consecutive days of >110.
