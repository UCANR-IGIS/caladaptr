#' Climate variables
#'
#' Climate variables available as raster series
#'
#' @docType data
#'
#' @usage data(cvars)
#'
#' @format A character vector with the names of climate variables
#'
#' @keywords datasets
#'
#' @details The following climate variables are available as raster series thru the Cal-Adapt API.
#'
#' \code{tasmax}: Maximum Temperature (historical values from UW Hydro and forecast values from LOCA downscaled climate projections)
#'
#' \code{tasmin}: Minimum Temperature (historical values from UW Hydro and forecast values from LOCA downscaled climate projections)
#'
#' \code{pr}: Precipitation (historical values from UW Hydro and forecast values from LOCA downscaled climate projections)
#'
#' @source \url{https://berkeley-gif.github.io/caladapt-docs/data-catalog.html#climate-variables}
"cvars"

